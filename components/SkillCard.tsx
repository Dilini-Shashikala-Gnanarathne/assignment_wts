...
export const SkillCard: React.FC<SkillCardProps> = ({ skill, index }) => {
  return (
    <motion.div
      ...
      className="group relative flex flex-col items-center justify-center p-6 bg-linear-to-br from-white/5 to-white/10 backdrop-blur-sm rounded-2xl border border-white/10 hover:border-(--accent)/50 transition-all duration-300 cursor-pointer"
    >
      <div
      ...
      <p className="relative z-10 text-sm sm:text-base font-semibold text-white text-center group-hover:text-(--accent) transition-colors duration-300">
        {skill.name}
      </p>
