  ...
  return (
    ...
    >
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div className="absolute inset-0 bg-linear-to-t from-(--bg-0) via-(--bg-0)/60 to-transparent opacity-80 group-hover:opacity-90 transition-opacity duration-500" />
      </div>
          ...
          >
            {project.tags.map((tag, i) => (
              <span
                key={i}
                className={`
                  px-3 py-1 text-xs font-semibold rounded-full backdrop-blur-sm border
                  ${
                    i % 2 === 0
      ...
      <div className="absolute top-0 right-0 w-32 h-32 bg-linear-to-br from-(--accent)/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-2xl" />
    </motion.div>
  );
};
