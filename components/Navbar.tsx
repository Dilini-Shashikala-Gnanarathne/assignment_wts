...
export default function Navbar({ isOpen, onClose }: NavbarProps) {
  ...
  const socialIcons = [
    {
      icon: <Github size={24} />,
      link: "https://github.com/Dilini-Shashikala-Gnanarathne",
      label: "GitHub",
    },
    {
      icon: <Mail size={24} />,
      link: "mailto:shashikaladilini11@gmail.com",
      label: "Email",
    },
    {
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed top-0 right-0 h-full bg-linear-to-br from-(--bg-0) via-(--bg-1) to-(--bg-2) w-full sm:w-112.5 lg:w-125 z-50 flex flex-col p-8 sm:p-12 text-white overflow-y-auto"
          >
                  ...
                  >
                    ...
                    <span
                      ...
                      ${
                        isActive
                          ? "text-(--accent)"
                          : "text-white group-hover:text-(--accent) group-hover:translate-x-4"
                      }
                    `}
                    >
                      {link.label}
                    ...
                    <span
                      className={`
                      absolute bottom-2 left-0 h-1 bg-(--accent) transition-all duration-500
                    ...
                    {isActive && (
                      <motion.span
                        ...
                        className="absolute left-0 top-1/2 -translate-y-1/2 w-2 h-12 bg-(--accent) rounded-r-full"
                        transition={{
                          type: "spring",
                          stiffness: 300,
                          damping: 30,
              ...
              <div className="flex gap-4">
                {socialIcons.map((item, index) => (
                  <a
                    ...
                    className="group relative p-3 bg-white/5 hover:bg-(--accent-2) rounded-full transition-all duration-300 transform hover:scale-110"
                  >
